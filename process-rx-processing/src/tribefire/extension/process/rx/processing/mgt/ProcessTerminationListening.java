...
public class ProcessTerminationListening {
	...
	private final Map<Reference<? extends ProcessItem>, ItemListeners> individualListeners = new ConcurrentHashMap<>();
	private final Set<BiConsumer<Reference<? extends ProcessItem>, ProcessActivity>> listeners = ConcurrentHashMap.newKeySet();
	...
	public void addProcessListener(Reference<? extends ProcessItem> processRef, Consumer<ProcessActivity> listener) {
		individualListeners.compute(processRef, (k, v) -> computeAdd(v, listener));
	}

	public void removeProcessListener(Reference<? extends ProcessItem> processRef, Consumer<ProcessActivity> listener) {
		individualListeners.compute(processRef, (k, v) -> computeRemove(v, listener));
	}
	...
	private static class ItemListeners {
		private final List<Consumer<ProcessActivity>> listeners = new ArrayList<>();
	...
	private ItemListeners computeAdd(ItemListeners itemListeners, Consumer<ProcessActivity> listener) {
		if (itemListeners == null) {
	...
	private ItemListeners computeRemove(ItemListeners itemListeners, Consumer<ProcessActivity> listener) {
		if (itemListeners == null)
			return null;
